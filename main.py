...
# User models and conversation history
user_models = {}
...
class MyView(View):
    ...
    async def set_nachiket(self, interaction: discord.Interaction, button: Button):
        user_models[interaction.user.id] = 'nachiket'
    ...
    async def set_gemini(self, interaction: discord.Interaction, button: Button):
        user_models[interaction.user.id] = 'regular'
...
async def on_message(message):
    ...
    if message.content.strip() == '!start':
        ...
        model_choice = user_models[user_id]

        # Select appropriate model configuration
        if model_choice == 'nachiket':
...
MESSAGE HISTORY:s
"""
            # Build prompt with special personality
